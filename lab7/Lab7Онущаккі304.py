...
# Автор: (вкажіть своє прізвище)

def generate_jagged_array(size, fill_char):
    """
    Створює зубчастий масив:
    - квадрат із символів '*'
    - у центрі квадратна рамка з символів fill_char
    - середина цієї рамки пуста
    """
    jagged_array = []
    ...
    for i in range(size):
        ...
        for j in range(size):
            if (i == start or i == end) and (start <= j <= end):
                ...
                row.append('*')
        # перетворюємо список у зубчастий масив (рядки можуть мати різну довжину)
...
def main():
    ...
    jagged_array = generate_jagged_array(size, fill_char)
